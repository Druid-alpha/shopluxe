  ...
  return (
    <div className="flex items-center gap-0.5">
      ...
      {Array(fullStars).fill(0).map((_, idx) => (
        <span key={`full-${idx}`} style={{ ...baseStyle, color: '#facc15' }}>★</span>
      ))}

      {/* Half star */}
      {hasHalfStar && (
        <span style={{ ...baseStyle, color: '#facc15', position: 'relative' }}>
      ...
      {Array(emptyStars).fill(0).map((_, idx) => (
        <span key={`empty-${idx}`} style={{ ...baseStyle, color: '#d1d5db' }}>☆</span>
      ))}
    </div>
  )
}
